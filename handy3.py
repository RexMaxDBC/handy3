...
<style>
    ...
    .header-container {{ border: 2px solid #D3D3D3; border-radius: 12px; background-color: rgba(211, 211, 211, 0.15); display: flex; justify-content: center; padding: 10px; margin-bottom: 20px; }}
...
# --- INFO BEREICH ---
with st.expander("ℹ️ Was ist die Pomodoro-Methode?"):
    st.markdown("""
    ### Die Methode
    * **25 Min. Fokus:** Volle Konzentration auf eine Aufgabe.
    * **5 Min. Pause:** Kurze Erholung fürs Gehirn.
    * **Rhythmus:** Nach 4 Einheiten folgt eine lange Pause (15–30 Min.).
    
    ### Warum sie wirkt
    * **Leichter Start:** 25 Minuten schrecken weniger ab als stundenlanges Lernen.
    * **Frische im Kopf:** Pausen verhindern Erschöpfung und halten den Fokus scharf.
    * **Zeitgefühl:** Du lernst genau, wie viel Zeit du für Themen brauchst.
    
    **Der Clou:** Der **Handy-Wächter** sichert deine Fokus-Phase ab, indem er die größte Ablenkung eliminiert!
    """)
    ...
    if t in st.session_state.tasks:
        ...
        with c_close:
            if st.button("✕ Zu", use_container_width=True):
                ...
                st.rerun()
        with c_title:
            st.markdown(f"<div class='modal-title'>📚 {t}</div><div class='modal-subtitle'>Details & Bewertung</div>", unsafe_allow_html=True)
        st.markdown(f"<div class='progress-wrap'><div class='progress-fill' style='width:{pct_int}%'></div></div><div class='progress-label'>{done}/{target} Sessions ({pct_int}%)</div>", unsafe_allow_html=True)
        
        star_cols = st.columns(5)
        for i, col in enumerate(star_cols):
            num = i + 1
            icon = "⭐" if num <= task["stars"] else "☆"
            if col.button(f"{icon} {num}", key=f"star_{t}_{num}", use_container_width=True):
                task["stars"] = 0 if task["stars"] == num else num
                st.rerun()
        new_comment = st.text_area("Notiz", value=task["comment"], key=f"txt_{t}", label_visibility="collapsed")
...
if st.session_state.active and st.session_state.mode == "Pomodoro":
    ...
    with c2:
        if img_f:
            ...
            # Fehlalarm-Schutz: Nur reagieren bei 'cell phone' und Score > 0.85
            handy = any(r['label'] == 'cell phone' and r['score'] > 0.85 for r in results)
            if handy:
...
if st.session_state.active:
    time.sleep(0.5)
    st.rerun()    .modal-title {{ color: white; font-size: 1.6rem; font-weight: bold; margin-bottom: 4px; }}
    .modal-subtitle {{ color: rgba(255,255,255,0.65); font-size: 0.9rem; margin-bottom: 20px; }}
    .progress-wrap {{ background: rgba(255,255,255,0.15); border-radius: 20px; height: 14px; margin-bottom: 6px; overflow: hidden; }}
    .progress-fill {{ height: 100%; border-radius: 20px; background: linear-gradient(90deg, #56ab2f, #a8e063); transition: width 0.4s; }}
    .progress-label {{ color: rgba(255,255,255,0.75); font-size: 0.85rem; margin-bottom: 20px; }}
